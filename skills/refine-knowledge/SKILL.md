...
2. 用户确认后，执行：`bash <skills-root>/scripts/init-vault.sh <vault-path>`
   - `<skills-root>` 为本 skill 的上级目录（即 `../../`）
...
4. 如果用户确认是，执行：`bash <skills-root>/scripts/mount-hooks.sh <skills-root>`
